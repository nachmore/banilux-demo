    ...
    public static class ApplicationBuilderExtensions
    {
        public static void UseEmfMiddleware(this IApplicationBuilder app)
        {
            app.UseEmfMiddleware(
                (context, logger) =>
                {
                    logger.SetNamespace("BaniluxService");
                    var dimensions = new DimensionSet();
                    var dimensionsWithStatusCode = new DimensionSet();
                    ...
                    if (endpoint != null)
                    {
                        ...
                        dimensionsWithStatusCode.AddDimension(
                            "Controller",
        ...
        public static void UseEmfMiddleware(
            ...
            Func<HttpContext, IMetricsLogger, Task> action
        )
        {
            app.Use(
                async (context, next) =>
                {
                    ...
                    stopWatch.Stop();
                    logger.PutMetric("Time", stopWatch.ElapsedMilliseconds, Unit.MILLISECONDS);
                }
            );
        }
    }
