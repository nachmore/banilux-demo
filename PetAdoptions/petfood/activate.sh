...
deploy () {
    cd $HOME/environment/workshopfiles/banilux-demo/PetAdoptions/${service}
...
permissions () {
    ...
    if [ ! "`aws iam get-role-policy --role-name ${role} --policy-name evidently`" ] ; then
        ...
        aws iam put-role-policy --role-name ${role} --policy-name evidently --policy-document file://$HOME/environment/workshopfiles/banilux-demo/PetAdoptions/${service}/policy.json
    else
