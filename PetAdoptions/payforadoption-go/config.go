...
func fetchConfigFromParameterStore(region string) (payforadoption.Config, error) {
	...
	res, err := svc.GetParametersWithContext(ctx, &ssm.GetParametersInput{
		Names: []*string{
			aws.String("/banilux/updateadoptionstatusurl"),
			aws.String("/banilux/rdssecretarn"),
			aws.String("/banilux/s3bucketname"),
			aws.String("/banilux/dynamodbtablename"),
		},
	})
